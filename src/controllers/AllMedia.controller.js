...
function RunYtDpAll(url) {
  return new Promise((resolve, reject) => {
    // Only pass --cookies when the file is actually present, otherwise yt-dlp
    // aborts with "cookies file not found" for sites that need no auth.
    const cookieArg = fs.existsSync(config.cookiesPath)
      ? `--cookies ${JSON.stringify(config.cookiesPath)} `
      : "";

    const command = `"${ytdlp}" -j --no-warnings ${cookieArg}--format "best[ext=mp4]/best[ext=webm]/best" ${JSON.stringify(url)}`;
